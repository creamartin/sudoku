...
function Square(props) {
    ...
    return (
        <td className={classes} onClick={() => props.onClick(props.index)}>
            <div>
                <SvgClick value={props.value?props.value:""}/>
            </div>
            {notes}
        </td>
    );
...
function Board(props) {
    function renderSquare(i) {
        return (<Square key={i}
                    ...
                    isPartOfGroup={isPartOfGroup(i,props.selected)}
                    isSameAsSelected={isSameAsSelected(i,props.selected,props.squares)}
